...
const radiusScale = d3.scaleLinear().domain([0,2.5]).range([10, 140])
const strokeScale = d3.scaleLinear().domain([0,2.5]).range([1.5,4])
...
function ready([json]) {
  ...
  // add red arrows for positive values
  svg
    .append('defs')
    .append('marker')
    .attr('id', 'red-arrow')
  ...
  svg
    ...
    .attr('y2', function(d) {
      console.log(radiusScale(d.properties.REPPercChange))
      return path.centroid(d)[1] - radiusScale(Math.abs(d.properties.REPPercChange))
    })
    .attr('class', 'lines')
    .attr('opacity', 1)
    .attr('stroke', d => {
        if (d.properties.REPPercChange > 0) return '#D03240'
        else return '#36454F'
      })
    .attr("stroke-width", d => strokeScale(Math.abs(d.properties.REPPercChange)))
    // below will be redundant because those will be passed to line
    .attr('marker-end', d => {
      if (d.properties.REPPercChange > 0) return 'url(#red-arrow)'
      else return 'url(#gray-arrow)'
    })
    .attr('transform', function(d) {
      const rotate = d.properties.REPPercChange > 0 ? '0' : '180'
  ...
  labels
    .append('text')
    .text(function(d) {
      const gain = d.properties.REPPercChange > 0 ? '+' : ''
      return `${gain}${d3.format(",.1%")(parseFloat((d.properties.REPPercChange)))} votes`
    })
    .attr('transform', function(d) {
      return `translate(${path.centroid(d)})`
    })
