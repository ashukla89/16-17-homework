...
function ready([json]) {
  ...
  svg
    ...
    .attr('y2', function(d) {
      console.log(radiusScale(d.properties.REPVoteDelta))
      return path.centroid(d)[1] - radiusScale(Math.abs(d.properties.REPVoteDelta))
    })
    .attr('class', 'lines')
    .attr('opacity', 1)
    .attr('stroke', d => {
        if (d.properties.REPVoteDelta > 0) return '#D03240'
        else return '#36454F'
      })
    .attr("stroke-width", d => strokeScale(Math.abs(d.properties.REPVoteDelta)))
    // below will be redundant because those will be passed to line
    .attr('marker-end', d => {
      if (d.properties.REPVoteDelta > 0) return 'url(#red-arrow)'
      else return 'url(#gray-arrow)'
    })
    .attr('transform', function(d) {
      const rotate = d.properties.REPVoteDelta > 0 ? '0' : '180'
  ...
  labels
    .append('text')
    .text(function(d) {
      const gain = d.properties.REPVoteDelta > 0 ? '+' : ''
      return `${gain}${d3.format(",")(parseFloat((d.properties.REPVoteDelta)).toFixed(1))} votes`
    })
    .attr('transform', function(d) {
      return `translate(${path.centroid(d)})`
    })
