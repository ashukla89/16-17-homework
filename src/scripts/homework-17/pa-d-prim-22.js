...
function ready([json]) {
  ...
  svg
    ...
    .attr('y2', function(d) {
      console.log(radiusScale(d.properties.DEMPercChange))
      return path.centroid(d)[1] - radiusScale(Math.abs(d.properties.DEMPercChange))
    })
    .attr('class', 'lines')
    .attr('opacity', 1)
    .attr('stroke', d => {
        if (d.properties.DEMPercChange > 0) return '#1A6AFF'
        else return '#36454F'
      })
    .attr("stroke-width", d => strokeScale(Math.abs(d.properties.DEMPercChange)))
    // below will be redundant because those will be passed to line
    .attr('marker-end', d => {
      if (d.properties.DEMPercChange > 0) return 'url(#blue-arrow)'
      else return 'url(#gray-arrow)'
    })
    .attr('transform', function(d) {
      const rotate = d.properties.DEMPercChange > 0 ? '0' : '180'
  ...
  labels
    .append('text')
    .text(function(d) {
      const gain = d.properties.DEMPercChange > 0 ? '+' : ''
      return `${gain}${d3.format(",.1%")(parseFloat((d.properties.DEMPercChange)))} votes`
    })
    .attr('transform', function(d) {
      return `translate(${path.centroid(d)})`
    })
